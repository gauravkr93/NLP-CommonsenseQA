...
@register_task('commonsense_qa_with_kb')
class CommonsenseQAWithKBTask(FairseqTask):
        ...
        with open(data_path) as h:
            for line in h:
                ...
                for i, choice in enumerate(example['choices']):
                    src = 'A: ' + choice
                    # facts = 'K: ' + '. '.join(example['premises'][i])
                    src += '. ' + '. '.join(example['premises'][i])
                    # src_bin = torch.cat([question_toks, binarize(src), binarize(facts)])
                    src_bin = torch.cat([question_toks, binarize(src)])
                    src_tokens[i].append(src_bin[:512])  # truncate to 512 if necessary
